...
class DmaChannel(Protocol):
    ...
    # Channel error (INE_ERR, INT_ERR)
    error: zdc.Event = zdc.field()
    # Transfer complete (INE_DONE, INT_DONE)
    done: zdc.Event = zdc.field()
    # Chunk complete (INE_CHK_DONE, INT_CHK_DONE)
...
class DmaGlobalRegs(zdc.RegFile):
    ...
    0x0C - INT_SRC_A: Interrupt Source A (shows which channels assert
        inta_o)
    0x10 - INT_SRC_B: Interrupt Source B (shows which channels assert
        intb_o)
    """
...
class DmaMMIO(Protocol):
    ...
    This defines the complete MMIO-level interface for the WISHBONE
    DMA/Bridge.
    ...
    - Each channel is a self-contained unit with its own registers and
        events
    ...
    Example usage:
        ...
        csr = DmaChannelCSR(ch_en=1, inc_src=1, inc_dst=1)
        await dma.ch0.regs.csr.write(csr)
