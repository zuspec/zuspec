...
from ..mmio import DmaMMIO, DmaChannelDesc
...
class DmaMMIOAlg(zdc.Component):
    ...
    # Arbitration state
    memif_lock: zdc.Lock = zdc.inst()  # Serialize access to memory
    channel_states: Tuple[ChannelState, ...] = zdc.tuple(
        size=4, elem_factory=ChannelState)
    ...
    async def _load_descriptor(self, idx: int, desc_addr: zdc.u32):
        ...
        # eol = (desc_csr >> 20) & 0x1  # EOL handled in _complete_channel
