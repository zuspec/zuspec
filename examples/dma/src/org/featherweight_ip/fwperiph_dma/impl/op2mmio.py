...
class DmaChannelOp2MMIO(DmaChannelOp, zdc.Component):
    ...
    async def transfer(self,
        ...
        Programs the channel registers for a simple block copy without chunking.
        Sets CHK_SZ=0 so the entire transfer completes atomically.
        """
        ...
        if result_csr.err:
            raise RuntimeError(f"DMA transfer error on channel {self._get_channel_idx()}")
    ...
    async def transfer_hw_handshake(self,
        ...
        # Wait for completion (unless auto_restart, in which case this runs forever)
        if not auto_restart:
            await ch.done.wait()
    ...
    async def setup_circular_buffer(self,
        ...
        assert buffer_size_bytes >= 16, "Buffer size must be at least 16 bytes"
        assert (buffer_size_bytes & (buffer_size_bytes - 1)) == 0, "Buffer size must be power of 2"
    ...
    async def setup_fifo_buffer(self,
        ...
        to prevent DMA from crossing the pointer (preventing overrun/underrun).
        """
        ch = self._get_mmio_channel()
        
        # First set up circular buffer
        await self.setup_circular_buffer(base_addr, buffer_size_bytes, interface, is_source)
    ...
    async def transfer_linked_list(self,
        ...
        if chunk_bytes is not None:
            ...
            await ch.regs.sz.write(DmaChannelSZ(tot_sz=sz.tot_sz, chk_sz=chk_sz))
...
class DmaOp2MMIO(DmaOp, zdc.Component):
    ...
    channels : Tuple[DmaChannelOp, ...] = zdc.tuple(size=4, elem_factory=DmaChannelOp2MMIO)
