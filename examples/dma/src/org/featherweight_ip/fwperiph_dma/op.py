...
class DmaChannelOp(Protocol):
    ...
    async def transfer_chunked(self,
                               src : zdc.u32,
                               dst : zdc.u32,
                               size_bytes : zdc.u32,
                               chunk_bytes : zdc.u32,
        ...
        between chunks. This allows fair bandwidth distribution across channels
        and lower latency for higher priority channels.
        """
        ...

    async def transfer_hw_handshake(self,
                                    src : zdc.u32,
                                    dst : zdc.u32,
                                    size_bytes : zdc.u32,
                                    chunk_bytes : zdc.u32,
        ...
        Args:
            ...
            dst: Destination address (word-aligned)
            size_bytes: Total transfer size in bytes (must be multiple of 4)
            chunk_bytes: Chunk size per trigger in bytes (must be multiple of 4)
            src_interface: Source WISHBONE interface (0 or 1)
            dst_interface: Destination WISHBONE interface (0 or 1)
            increment_src: Whether to increment source address
            increment_dst: Whether to increment destination address
    ...
    async def setup_circular_buffer(self,
                                    base_addr : zdc.u32,
        ...
        Args:
            ...
            buffer_size_bytes: Buffer size in bytes (min 16 bytes, must be power of 2)
    ...
    async def setup_fifo_buffer(self,
                                base_addr : zdc.u32,
                                buffer_size_bytes : zdc.u32,
                                sw_ptr : zdc.u32,
                                interface : zdc.u1 = 0,
        ...
        Args:
            ...
            buffer_size_bytes: Buffer size in bytes (min 16 bytes, must be power of 2)
            sw_ptr: Initial software pointer (last location read/written by SW)
            interface: WISHBONE interface (0 or 1)
            is_source: True for source buffer, False for destination buffer
    ...
    async def transfer_linked_list(self,
                                   desc_addr : zdc.u32,
...
class DmaOp(Protocol):
    ...
    Provides access to all DMA channels. Each channel can operate independently
    and concurrently. This reflects the hardware architecture where multiple
    channels can be active simultaneously, with the arbiter selecting which
    channel accesses the WISHBONE interfaces based on priority.
    
    Example usage:
        # Perform transfers on multiple channels concurrently
        async def copy_data(dma):
            # Start transfers on multiple channels
            await dma.channels[0].transfer(src=0x1000, dst=0x2000, size_bytes=256)
            await dma.channels[1].transfer(src=0x3000, dst=0x4000, size_bytes=512)
    ...
    """
    channels : Tuple[DmaChannelOp, ...] = zdc.tuple(size=4)
